...
public sealed class CityLawManager : Component
{
	...
	public void RequestAddLaw( string lawText )
	{
		...
		{
			Notices.SendNotice( Rpc.Caller, "block", Color.Red, "Enter a border law first.", 3 );
			return;
		}
		...
		{
			Notices.SendNotice( Rpc.Caller, "block", Color.Red, $"The border can only have {MaxLaws} laws.", 3 );
			return;
		}

		if ( laws.Any( x => string.Equals( x, law, StringComparison.OrdinalIgnoreCase ) ) )
		{
			Notices.SendNotice( Rpc.Caller, "block", Color.Red, "That border law already exists.", 3 );
			return;
		}
