...
public sealed class Lawboard : Component, Component.IPressable
{
	...
	IPressable.Tooltip? IPressable.GetTooltip( IPressable.Event e )
	{
		...
		var description = laws is { Count: > 0 }
			? $"{laws.Count} border law{(laws.Count == 1 ? "" : "s")} posted."
			: "No border laws posted.";

		return new IPressable.Tooltip( "Border Laws", "gavel", description );
	}
	...
	static string BuildBoardText( IReadOnlyList<string> laws )
	{
		...
		var lines = new List<string> { "BORDER LAWS", "" };
